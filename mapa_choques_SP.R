...
registro_accidentes <- read_csv("~/ACCIDENTES_CRUCEROS_SAN_PEDRO.csv")%>%
  rename_all(iconv) %>%
  mutate(calle1 = iconv(calle1),
         calle2 = iconv(calle2),
         fechaCorte = as.Date(fechaCorte, format = "%d/%m/%Y"))

head(registro_accidentes)  

#Quedarnos con los nombres de los cruces para despues pegar sus coordenadas
nombres_cruces <- registro_accidentes %>%
  distinct(calle1,calle2)%>%
  arrange(calle1, calle2)

coordenadas_cruces <- data.frame(
  calle1 = nombres_cruces$calle1,
  calle2 = nombres_cruces$calle2,
  latitud = c(25.65716, 25.65381, 25.65945, 25.67473, 25.67048,
  ...
  longitud = c(-100.37038, -100.35807, -100.36950, -100.40281, -100.40226,
  ...
  mutate(calle1 = as.character(calle1),
         calle2 = as.character(calle2))

#Obtener la cantidad total de choques por cruce.
cantidad_por_cruce <- registro_accidentes %>%
...
#Pegar el resumen estadístico con las coordenadas. Popup es para el despliegue en el mapa.
reporte_cruces <- left_join(cantidad_por_cruce, coordenadas_cruces, by = c("calle1", "calle2"))%>%
...
reporte_cruces_sf <- st_as_sf(x = reporte_cruces, 
